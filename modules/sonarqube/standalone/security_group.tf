locals {
  security_group_name = "sec-${var.region_name}-${var.solution_fqn}-sonarqube"
}

resource aws_security_group sonarqube {
  name = local.security_group_name
  description = "Controls access to the Sonarqube service"
  vpc_id = data.aws_subnet.given.vpc_id
  tags = merge({ Name : local.security_group_name }, local.module_common_tags)
}

resource aws_security_group_rule sonarqube_http_in {
  type = "ingress"
...
resource aws_security_group_rule sonarqube_ssh_in {
  type = "ingress"
...
resource aws_security_group_rule sonarqube_any_out {
  type = "egress"
