locals {
  ec2_instance_name = "ec2-${var.region_name}-${var.solution_fqn}-opsbox"
}
...
resource aws_instance opsbox {
  ...
  ebs_optimized = true
  # hibernation = true
  iam_instance_profile = aws_iam_instance_profile.opsbox.id
  ...
  root_block_device {
    ...
    volume_type = "gp3"
    tags = merge({
      Name = "vol-${var.region_name}-${var.solution_fqn}-opsbox-root"
